...
class EPub extends EventEmitter {
        ...
        xmlparser.on(
          ...
          function (err) {
            this.emit(
              "error",
              new Error(
                "Parsing container XML failed in getRootFiles: " + err.message
              )
            );
            return;
          }.bind(this)
        ...
        xmlparser.on(
          ...
          function (err) {
            this.emit(
              "error",
              new Error(
                "Parsing container XML failed in handleRootFile: " + err.message
              )
            );
            return;
          }.bind(this)
      ...
      switch (key) {
        case "publisher":
          if (Array.isArray(metadata[keys[i]])) {
            this.metadata.publisher = String(
  ...
  parseTOC() {
    var i,
      len,
      path = this.spine.toc.href.split("/"),
      id_list = {},
      keys;
    path.pop();
      ...
      function (err, data) {
        ...
        xmlparser.on(
          ...
          function (err) {
            this.emit(
              "error",
              new Error("Parsing container XML failed in TOC: " + err.message)
            );
            return;
          }.bind(this)
        );
    ...
    for (var i = 0; i < branch.length; i++) {
      ...
      if (branch[i].navPoint) {
        output = output.concat(
          this.walkNavMap(branch[i].navPoint, path, id_list, level + 1)
        );
      }
    }
    return output;
